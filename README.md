...
[[분류:인물]]                  → 이 문서를 분류에 넣습니다(본문에서 빠지고 문서 아래 분류 줄로 모임)
[[연표:2024.3|정식 출시]]      → 사건 등록(본문에는 설명 글자만 남음)
```
...
위키 게시판은 글 목록이 보통 게시판과 다르게 보입니다.

| 상황 | 목록에 나오는 것 |
|---|---|
| 그냥 들어갔을 때 | **대문 글 1건**만 (대문을 지정하지 않았으면 보통 게시판처럼 전부) |
| 검색창에 검색했을 때 | 제목에 그 글자가 **들어 있는** 문서. 한 글자도 됩니다 |

- 정렬은 **문서 이름순**입니다(정규화 제목 오름차순).
- 검색 대상(제목·내용·작성자)을 무엇으로 고르든 **제목 검색**으로 처리합니다. 위키에서는
  제목이 곧 문서 이름이기 때문입니다.
- `%` 나 `_` 를 검색해도 **글자 그대로** 찾습니다(아무거나 걸리는 와일드카드가 아닙니다).
- **공지 글도 함께 걸러집니다.** 코어가 공지를 같은 목록 앞에 섞어 보내기 때문에, 목록을
  갈아 끼우면 공지도 빠집니다. 위키 게시판에는 공지를 쓰지 않는 편이 좋습니다.
- 목록 항목 자체(제목·작성자·날짜·댓글 수·비밀글 자물쇠…)는 **코어가 만든 그대로**입니다.
  이 플러그인은 "어떤 글을 담을지" 만 바꿉니다.
